...
import './InteractiveLogoutButton.css';
...
export default function InteractiveLogoutButton({ onLogout, className = '', sidebarOpen = true }: InteractiveLogoutButtonProps) {
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      if (stateRef.current === 'default' || stateRef.current === 'hover') {
        ...
        // Execute the actual logout slightly before the animation fully ends
        // but after the guy falls, so they see the joke!
        setTimeout(() => {
          onLogout();
        }, 1200);
  ...
  return (
    <button ref={buttonRef} className={`logoutButton ${className} ${sidebarOpen ? '' : '!w-12 !px-0 flex justify-center'}`}>
      <svg className={`doorway ${sidebarOpen ? '' : 'hidden'}`} viewBox="0 0 100 100">
      ...
      <svg className={`figure ${sidebarOpen ? '' : '!right-auto !relative'}`} viewBox="0 0 100 100">
      ...
      <svg className={`door ${sidebarOpen ? '' : 'hidden'}`} viewBox="0 0 100 100">
        <path d="M93.4 86.3H58.6c-1.9 0-3.4-1.5-3.4-3.4V17.1c0-1.9 1.5-3.4 3.4-3.4h34.8c1.9 0 3.4 1.5 3.4 3.4v65.8c0 1.9-1.5 3.4-3.4 3.4z" />
        <circle cx="66" cy="50" r="3.7" />
      </svg>
      {sidebarOpen && <span className="button-text">Log Out</span>}
    </button>
  );
}
