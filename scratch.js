...
.star-wars-toggle .side.light-side {
  ...
  --y: calc(Math.sin(var(--angle)) * var(--radius) * -1);
  --x: calc(Math.cos(var(--angle)) * var(--radius) * -1);
...
.star-wars-toggle .side.dark-side {
  ...
  --angle: calc(180deg * (var(--progress, 0)));
  --radius: calc(var(--size) / 2);
  --y: calc(Math.sin(var(--angle)) * var(--radius));
  --x: calc(Math.cos(var(--angle)) * var(--radius));
  transform: translateZ(var(--x)) translateX(var(--y)) rotateY(calc(var(--angle)));
}
...
fs.writeFileSync('c:\\follow\\src\\components\\StarWarsToggle.css', css);
`;
fs.writeFileSync('c:\\follow\\scratch.js', css);
